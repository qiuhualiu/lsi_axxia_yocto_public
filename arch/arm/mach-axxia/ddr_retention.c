...
static inline void ncp_ddr_shutdown(void)
{
    ...
    for (smId = 0; smId < 2; smId++) {
        /* CDR2 - Node.target */
        ncr_register_write(cdr2[smId], (unsigned *) (nca_address + 0xf8));
        /* CDR0 - */
        ncr_register_write(0x80050003, (unsigned *) (nca_address + 0xf0));
        do {
            kill_time(100000);
            value = ncr_register_read((unsigned *)(nca_address + 0xf0));
    ...
    for (smId = 0; smId < 2; smId++) {
        ...
        ncr_register_write(cdr2[smId], (unsigned *) (nca_address + 0xf8));
        do { 
            ncr_register_write(loop, (unsigned *) (nca_address + 0x11f0));

            /* issue config ring read */
            ncr_register_write(0x80040003, (unsigned *) (nca_address + 0xf0));
            do {
                kill_time(100000);
                value = ncr_register_read((unsigned *)(nca_address + 0xf0));
            } while ((0x80000000UL & value));

            value = ncr_register_read((unsigned *)(nca_address + 0x1000));
            ncr_register_write(value, (unsigned *) (nca_address + 0x1200));
...
static ssize_t axxia_ddr_retention_trigger(struct file *file, const char __user *buf,
