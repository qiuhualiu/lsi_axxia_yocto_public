...
#define AXXIA_PER_CLOCK  266000000
#define AXXIA_MMC_CLOCK   25000000
...
axxia_init_clocks(void)
{
	...
	if (!np || of_property_read_u32(np, "frequency", &frequency)) {
		pr_warn("axxia: Missing /clocks/cpu/frequency\n");
		frequency = AXXIA_CPU_CLOCK;
	}
	...
	if (!np || of_property_read_u32(np, "frequency", &frequency)) {
		pr_warn("axxia: Missing /clocks/peripheral/frequency\n");
		frequency = AXXIA_PER_CLOCK;
	}
	...
	if (!np || of_property_read_u32(np, "frequency", &frequency)) {
		pr_warn("axxia: Missing /clocks/emmc/frequency\n");
		frequency = AXXIA_MMC_CLOCK;
	}
