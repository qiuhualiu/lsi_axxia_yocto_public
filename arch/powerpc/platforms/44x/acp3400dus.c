...
static __initdata struct of_device_id acp3400dus_of_bus[] = {
	{ .compatible = "ibm,plb4", },
	{ .compatible = "ibm,plb6", },
	{ .compatible = "ibm,opb", },
	{ .compatible = "ibm,ebc", },
...
static int __init acp3400dus_device_probe(void)
{
	of_platform_bus_probe(NULL, acp3400dus_of_bus, NULL);

	if (acp_rio_sysfs_init())
		pr_err("ACP: rio hotplug sysfs failed\n");

	return 0;
}
machine_device_initcall(acp3400dus, acp3400dus_device_probe);

/* We can have either UICs or MPICs */
static void __init acp3400dus_init_irq(void)
{
...
#ifdef CONFIG_SMP
static void __cpuinit smp_acp3400dus_setup_cpu(int cpu)
{
	mpic_setup_this_cpu();
}

static int __cpuinit smp_acp3400dus_kick_cpu(int cpu)
{
...
static struct smp_ops_t acpx1_smp_ops = {
	...
	.setup_cpu	= smp_acp3400dus_setup_cpu,
	.kick_cpu	= smp_acp3400dus_kick_cpu,
	.give_timebase	= smp_generic_give_timebase,
	.take_timebase	= smp_generic_take_timebase,
};

static void __init acp3400dus_smp_init(void)
{
...
static void __init acp3400dus_setup_arch(void)
{
	acp3400dus_smp_init();
}

static void __devinit acp3400dus_pcibios_after_init(void)
{
	struct device_node *np;
 ...
 */
static int __init acp3400dus_probe(void)
{
...
static void __init acp3400dus_init(void)
{
	unsigned long val;
 ...
 */
static void __init acp3400dus_init_early(void)
{
 ...
 */
pgprot_t acp3400dus_phys_mem_access_prot(struct file *file, unsigned long pfn,
...
int acp3400dus_mcheck_exception(struct pt_regs *regs)
{
	u32 esr;
	u32 mcsr;
...
define_machine(acp3400dus) {
	.name			= "ACP3400DUS",
	.probe			= acp3400dus_probe,
	.init                   = acp3400dus_init,
	.init_early             = acp3400dus_init_early,
	.progress		= udbg_progress,
	.init_IRQ		= acp3400dus_init_irq,
	.setup_arch		= acp3400dus_setup_arch,
	.pcibios_after_init     = acp3400dus_pcibios_after_init,
	.restart		= ppc4xx_reset_system,
	.calibrate_decr		= generic_calibrate_decr,
	.phys_mem_access_prot   = acp3400dus_phys_mem_access_prot,
	.machine_check_exception = acp3400dus_mcheck_exception,
};
