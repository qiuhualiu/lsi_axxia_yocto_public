...
static __initdata struct of_device_id rbs_acp34xx_of_bus[] = {
	{ .compatible = "ibm,plb4", },
	{ .compatible = "ibm,plb6", },
	{ .compatible = "ibm,opb", },
	{ .compatible = "ibm,ebc", },
...
static int __init rbs_acp34xx_device_probe(void)
{
	pr_info("RBS_ACP34xx: %s\n", __func__);
	of_platform_bus_probe(NULL, rbs_acp34xx_of_bus, NULL);

	return 0;
}
machine_device_initcall(acp34xxduw2, rbs_acp34xx_device_probe);
machine_device_initcall(acp34xxdus41, rbs_acp34xx_device_probe);
machine_device_initcall(acp34xxdus41tb, rbs_acp34xx_device_probe);
machine_device_initcall(acp34xxtp1, rbs_acp34xx_device_probe);
machine_device_initcall(acp34xxtp5, rbs_acp34xx_device_probe);
machine_device_initcall(acp34xxduw31, rbs_acp34xx_device_probe);

/* We can have either UICs or MPICs */
static void __init rbs_acp34xx_init_irq(void)
{
...
#ifdef CONFIG_SMP
static void __cpuinit smp_rbs_acp34xx_setup_cpu(int cpu)
{
	mpic_setup_this_cpu();
}

static int __cpuinit smp_rbs_acp34xx_kick_cpu(int cpu)
{
...
static struct smp_ops_t acpx1_smp_ops = {
	...
	.setup_cpu	= smp_rbs_acp34xx_setup_cpu,
	.kick_cpu	= smp_rbs_acp34xx_kick_cpu,
	.give_timebase	= smp_generic_give_timebase,
	.take_timebase	= smp_generic_take_timebase,
};

static void __init rbs_acp34xx_smp_init(void)
{
...
static void __init rbs_acp34xx_setup_arch(void)
{
	rbs_acp34xx_smp_init();
}

static void __devinit rbs_acp34xx_pcibios_after_init(void)
{
	struct device_node *np;
 ...
 */
static int __init acp34xxdus41tb_probe(void)
{
	unsigned long root = of_get_flat_dt_root();

	if (!of_flat_dt_is_compatible(root, "acp34xxdus41tb"))
		return 0;

	return 1;
}
static int __init acp34xxdus41_probe(void)
{
	unsigned long root = of_get_flat_dt_root();

	if (!of_flat_dt_is_compatible(root, "acp34xxdus41"))
		return 0;

	return 1;
}
static int __init acp34xxduw41_probe(void)
{
	unsigned long root = of_get_flat_dt_root();
...
static int __init acp34xxtp1_probe(void)
{
	unsigned long root = of_get_flat_dt_root();

	if (!of_flat_dt_is_compatible(root, "acp34xxtp1"))
		return 0;

	return 1;
}
static int __init acp34xxtp5_probe(void)
{
	unsigned long root = of_get_flat_dt_root();

	if (!of_flat_dt_is_compatible(root, "acp34xxtp5"))
		return 0;

	return 1;
}
static int __init acp34xxduw31_probe(void)
{
	unsigned long root = of_get_flat_dt_root();

	if (!of_flat_dt_is_compatible(root, "acp34xxduw31"))
		return 0;
...
static void __init rbs_acp34xx_init(void)
{
	unsigned long val;
 ...
 */
static void __init rbs_acp34xx_init_early(void)
{
...
int rbs_acp34xx_mcheck_exception(struct pt_regs *regs)
{
	u32 esr;
	u32 mcsr;
...
define_machine(acp34xxduw2) {
	.name			= "ACP3400DUW41",
	.probe			= acp34xxduw41_probe,
	.init                   = rbs_acp34xx_init,
	.init_early             = rbs_acp34xx_init_early,
	.progress		= udbg_progress,
	.init_IRQ		= rbs_acp34xx_init_irq,
	.setup_arch		= rbs_acp34xx_setup_arch,
	.pcibios_after_init     = rbs_acp34xx_pcibios_after_init,
