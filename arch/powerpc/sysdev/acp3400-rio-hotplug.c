...
static void acp_rio_hotswap_work(struct work_struct *work)
{
	...
	if (RIO_EXTRACT_LP(flags)) {
		if (rio_port_started(mport)) {
			dev_info(priv->dev,
				 "Link partner is active - Hotswap extract aborted\n");
			goto out;
		}
		dev_dbg(priv->dev,
	...
	if (RIO_INSERT_LP(flags)) {
		if (rio_port_stopped(mport)) {
			dev_info(priv->dev,
				 "Link partner is not active - Hotswap insert aborted\n");
			goto out;
		}
		dev_dbg(priv->dev,
	...
	if (RIO_INSERT_MP(flags)) {
		...
		rio_mp_insert(mport);
		acp3400_rio_set_mport_disc_mode(mport);
		acp3400_rio_port_irq_enable(mport);
	}
out:
	dev_dbg(priv->dev,
		 "Port work done\n");
...
int acp3400_rio_hotswap(struct rio_mport *mport, u8 flags)
{
	struct rio_priv *priv = mport->priv;
	struct completion cmp;
