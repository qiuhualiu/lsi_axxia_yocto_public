					  ...
					  struct resource *res)
{
	...
	if (size < total_memory) {
		printk(KERN_ERR
		       "%s: dma-ranges too small (size=%llx total_memory=%llx)\n",
...
#if defined(CONFIG_ACP) && !defined(CONFIG_ACPISS)
...
static void ppc405ex_pcie_phy_reset(struct ppc4xx_pciex_port *port)
{
	...
	/* poll for phy !reset */
	/* XXX FIXME add timeout */
	while (!(mfdcri(SDR0, port->sdr_base + PESDRn_405EX_PHYSTA) &
	       0x00001000))
		;
...
static void
__init ppc4xx_pciex_port_init_mapping(struct ppc4xx_pciex_port *port)
{
...
static void
__iomem *ppc4xx_pciex_get_config_base(struct ppc4xx_pciex_port *port,
				      struct pci_bus *bus,
				      unsigned int devfn)
{
	int relbus;
				    ...
				    int offset, int len, u32 *val)
{
	...
	pr_debug(
		 "pcie-config-read: bus=%3d [%3d..%3d] devfn=0x%04x offset=0x%04x len=%d, addr=0x%p val=0x%08x\n",
				     ...
				     int offset, int len, u32 val)
{
	...
	pr_debug(
		 "pcie-config-write: bus=%3d [%3d..%3d] devfn=0x%04x offset=0x%04x len=%d, addr=0x%p val=0x%08x\n",
		 bus->number, hose->first_busno, hose->last_busno,
		 devfn, offset, len, addr + offset, val);
					     ...
					     int index)
{
	...
	case 0:
		...
		else if (of_device_is_compatible(port->node,
					       ...
					       struct resource *res)
{
	...
	if (port->endpoint) {
		...
		if (of_device_is_compatible(port->node,
					    "ibm,plb-pciex-460sx") ||
		    of_device_is_compatible(port->node,
					    "ibm,plb-pciex-476fpe"))
...
static void __init ppc4xx_probe_acp_pciex_bridge(struct device_node *np)
{
	...
	port->utl_base = (void __iomem *)
			 ((u32) (port->utl_regs.start - 0x2000400000ULL)
			 + 0xf0000000);
