...
#ifndef _ACP3400_RIO_H_
#define _ACP3400_RIO_H_
...
#define  EPC_PNADIDCSR_ADID(id)	((u32)((id) & 0xff) << 16)
...
#define DESC_TABLE_W0_BASE(d) (0x10000 + (0x10 * (d)))
#define DESC_TABLE_W0(d) (RAB_REG_BASE + (DESC_TABLE_W0_BASE(d)))
#define DESC_TABLE_W1(d) (RAB_REG_BASE + (0x10004 + (0x10 * (d))))
#define DESC_TABLE_W2(d) (RAB_REG_BASE + (0x10008 + (0x10 * (d))))
#define DESC_TABLE_W3(d) (RAB_REG_BASE + (0x1000C + (0x10 * (d))))
...
struct rio_priv {
	...
	struct rio_irq_handler ob_dme_irq[DME_MAX_OB_ENGINES];
	struct rio_irq_handler ib_dme_irq[RIO_MAX_RX_MBOX];

#ifdef CONFIG_ACP_RIO_STAT
	atomic_t event[RIO_EVENT_NUM];
	atomic_t state[RIO_STATE_NUM];
	atomic_t irq[RIO_IRQ_NUM];
	atomic_t ib_dme[DME_MAX_IB_ENGINES][RIO_IB_DME_NUM];
	atomic_t ob_dme[DME_MAX_OB_ENGINES][RIO_OB_DME_NUM];
#endif
	/* Fatal err */
...
extern int acp3400_rio_start_port(struct rio_mport *mport);
extern void acp3400_rio_set_mport_disc_mode(struct rio_mport *mport);
extern void acp3400_rio_static_win_release(struct rio_mport *mport);
extern int acp3400_rio_static_win_init(struct rio_mport *mport);

#ifdef CONFIG_RAPIDIO_HOTPLUG

extern int acp3400_rio_hotswap(struct rio_mport *mport, u8 flags);
