...
static DEVICE_ATTR(ib_dme_time, S_IWUGO|S_IRUGO, acp_rio_ib_dme_time_show, acp_rio_ib_dme_time_store);
#endif
			   ...
			   size_t count)
{
	...
	if (rc == 1) {
		acp3400_rio_apio_disable(mport);
		rc = acp3400_rio_apio_enable(mport, 0, mask);
		if (!rc)
			return count;
	}
	return rc;
			    ...
			    size_t count)
{
	...
	if (rc == 1) {
		...
		if (priv->apio_irq.irq_state_mask != mask) {
			rc = acp3400_rio_apio_enable(mport, mask, 0);
			if (!rc)
				return count;
		}
	}
			   ...
			   size_t count)
{
	...
	if (rc == 1) {
		acp3400_rio_rpio_disable(mport);
		rc = acp3400_rio_rpio_enable(mport, 0, mask);
		if (!rc)
			return count;
	}
	return rc;
			    ...
			    size_t count)
{
	...
	if (rc == 1) {
		...
		if (priv->rpio_irq.irq_state_mask != mask) {
			rc = acp3400_rio_rpio_enable(mport, mask, 0);
			if (!rc)
				return count;
		}
	}
...
static struct ob_dme_stat ob_stat[3];
static struct ob_dme_stat ib_stat[8];
...
{
	struct rio_mport *mport = dev_get_drvdata(dev);
	int mbox, mbox_dest, letter;
	u16 dest_id;
	u32 flags;
	int size;
	int rc = sscanf(buf, "%d %d %hu %d %x %d",
			&mbox, &mbox_dest, &dest_id, &letter, &flags, &size);
...
int acp3400_rio_init_sysfs(struct platform_device *dev)
{
	return sysfs_create_group(&dev->dev.kobj, &rio_attribute_group);
}
void acp3400_rio_release_sysfs(struct platform_device *dev)
{
