...
DEFINE_MUTEX(acp3400_rio_api_mutex);
...
static irqreturn_t thrd_irq_handler(int irq, void *data)
{
	...
	/**
	 * Get current interrupt state and
	 * clear latched state for interrupts
 ...
 */
static int alloc_irq_handler(struct rio_irq_handler *h,
			     void *data,
			     const char *name)
{
	struct rio_mport *mport = h->mport;
...
/**
 * pw_irq_handler - ACP3400 port write interrupt handler
 * @h: handler specific data
...
static void acp_rio_flush_pw(struct rio_mport *mport, int noofpw,
...
static int enable_pw(struct rio_irq_handler *h)
{
	...
	rval |= RAB_IB_PW_EN;
	acp_rio_flush_pw(mport, RAB_IB_PW_NUMWORDS(rval), pw_data);
...
/**
 * acp_rio_tx_db_int_handler - ACP3400 inbound doorbell interrupt handler
 ...
 *
 * Handles outbound message interrupts. Executes a callback, if available,
 * on each successfully sent message.
 *
 * @note:
 ...
 * completed transactions while processing finished transactions may break
...
/**
 * open_outb_mbox - Initialize ACP3400 outbound mailbox
			  ...
			  int entries, int prio)
{
	...
	me = alloc_message_engine(mport, mbox, dev_id, buf_sz, entries, 1);
	if (IS_ERR(me))
		return PTR_ERR(me);

	do {
		__rio_local_read_config_32(mport,
					   RAB_OB_DME_STAT(mbox), &dme_stat);
		if (wait++ > 100) {
			rc = -EBUSY;
			goto err;
		}
...
/**
 * close_outb_mbox - Close ACP3400 outbound mailbox
 * @mport: Master port implementing the outbound message unit
...
static void release_ob_mbox(struct rio_irq_handler *h)
{
	struct rio_mport *mport = h->mport;
	struct rio_priv *priv = mport->priv;
...
/**
 * ib_dme_irq_handler - ACP3400 inbound message interrupt handler
 * @mport: Master port with triggered interrupt
 * @mask: Interrupt register data
 *
...
static void ib_dme_irq_handler(struct rio_irq_handler *h, u32 state)
{
	...
	int dme_no = 31 - CNTLZW(dme_mask);
		...
		BUG_ON(letter < 0);
		me = mb->me[letter];
		mbox_no = me->dme_no/RIO_MSG_MAX_LETTER;
		/**
		...
		if (atomic_read(&h->start_time)) {
			if (me->pkt == 0) {
				me->start_irq_tb = h->irq_tb;
				me->start_thrd_tb = h->thrd_tb;
			}
			me->stop_irq_tb = h->irq_tb;
			me->stop_thrd_tb = h->thrd_tb;
			if ((h->thrd_tb - h->irq_tb) > me->max_lat)
				me->max_lat = h->thrd_tb - h->irq_tb;
			if ((h->thrd_tb - h->irq_tb) < me->min_lat)
				me->min_lat = h->thrd_tb - h->irq_tb;
		}
#endif
		/**
...
/**
 * open_inb_mbox - Initialize ACP3400 inbound mailbox
...
static void release_ib_mbox(struct rio_irq_handler *h)
{
	struct rio_mport *mport = h->mport;
	struct rio_priv *priv = mport->priv;
	struct rio_rx_mbox *mb = h->data;
...
void acp3400_rio_port_get_state(struct rio_mport *mport, int cleanup)
{
	struct rio_priv *priv = mport->priv;
...
/**
 * acp3400_rio_port_irq_enable - Register RIO interrupt handler
 *
 * @mport: master port
 ...
 * <0       Failure
 */
int acp3400_rio_port_irq_enable(struct rio_mport *mport)
{
	struct rio_priv *priv = mport->priv;
	int rc;
	 ...
	 */
	acp3400_rio_port_get_state(mport, 1);
...
void acp3400_rio_port_irq_disable(struct rio_mport *mport)
{
	struct rio_priv *priv = mport->priv;
	int i;
...
int acp3400_rio_pw_enable(struct rio_mport *mport, int enable)
{
	struct rio_priv *priv = mport->priv;
	int rc = 0;
...
/**
 * acp_rio_doorbell_send - Send a doorbell message
 *
 ...
 *
 * Sends a doorbell message. Returns %0 on success or
 * %-EINVAL on failure.
 *
 * API protected by spin lock in generic rio driver.
 */
int acp3400_rio_doorbell_send(struct rio_mport *mport,
...
/************/
/**
 * acp3400_open_outb_mbox - Initialize ACP3400 outbound mailbox
 * @mport: Master port implementing the outbound message unit
 * @dev_id: Device specific pointer to pass on event
 * @mbox: Mailbox to open
 * @entries: Number of entries in the outbound mailbox ring for each letter
			   ...
			   int entries, int prio)
{
	...
	acp3400_api_lock();
	rc = open_outb_mbox(mport, dev_id, mbox, entries, prio);
	acp3400_api_unlock();
...
/**
 * acp3400_close_outb_mbox - Shut down ACP3400 outbound mailbox
 * @mport: Master port implementing the outbound message unit
 * @mbox: Mailbox to close
 *
 ...
 * frees the resources.
 */
void acp3400_close_outb_mbox(struct rio_mport *mport, int mbox)
{
	struct rio_priv *priv = mport->priv;
	...
	acp3400_api_lock();
	release_irq_handler(&priv->ob_dme_irq[mbox]);
	acp3400_api_unlock();
 ...
 *
 * Adds the @buffer message to the ACP3400 outbound message queue. Returns
 * %0 on success or %-EINVAL on failure.
 */
int acp3400_add_outb_message(struct rio_mport *mport, struct rio_dev *rdev,
			     ...
			     void *buffer, size_t len, void *cookie)
{
	...
	if ((len > RIO_MSG_SEG_SIZE) && (mbox_dest > 3))
		return -EINVAL;
...
/**
 * acp3400_open_inb_mbox - Initialize ACP3400 inbound mailbox
 * @mport: Master port implementing the inbound message unit
 * @dev_id: Device specific pointer to pass on event
 * @mbox: Mailbox to open
 * @entries: Number of entries in the inbound mailbox ring
...
/**
 * acp3400_close_inb_mbox - Shut down ACP3400 inbound mailbox
 * @mport: Master port implementing the inbound message unit
 * @mbox: Mailbox to close
 *
 ...
 * frees resources.
 */
void acp3400_close_inb_mbox(struct rio_mport *mport, int mbox)
{
	struct rio_priv *priv = mport->priv;
...
/**
 * acp3400_add_inb_buffer - Add buffer to the ACP3400 inbound message queue
 * @mport: Master port implementing the inbound message unit
 * @mbox: Inbound mailbox number
 * @buf: Buffer to add to inbound queue
 *
 * Adds the @buf buffer to the ACP3400 inbound message queue. Returns
 * %0 on success or %-EINVAL on failure.
 */
int acp3400_add_inb_buffer(struct rio_mport *mport, int mbox, void *buf)
{
	struct rio_priv *priv = mport->priv;
	struct rio_rx_mbox *mb;
	int rc = 0;

	BUG_ON((mbox < 0) || (mbox >= RIO_MAX_RX_MBOX));
 ...
 */
void *acp3400_get_inb_message(struct rio_mport *mport, int mbox, int letter,
			      int *sz, int *slot, u16 *destid)
{
	...
	BUG_ON((mbox < 0) || (mbox >= RIO_MAX_RX_MBOX));
	BUG_ON(letter >= RIO_MSG_MAX_LETTER);
...
void acp3400_rio_port_irq_init(struct rio_mport *mport)
{
	...
	for (i = 0; i < DME_MAX_OB_ENGINES; i++) {
		...
		priv->ob_dme_irq[i].release_fn = release_ob_mbox;
	}
	/**
	 * Inbound messages
	 */
	for (i = 0; i < RIO_MAX_RX_MBOX; i++) {
		...
		priv->ib_dme_irq[i].release_fn = release_ib_mbox;
	}
	/**
	 * PIO
...
#if defined(CONFIG_RAPIDIO_HOTPLUG)
int acp3400_rio_port_notify_cb(struct rio_mport *mport,
			       int enable,
...
int acp3400_rio_port_op_state(struct rio_mport *mport)
{
	u32 escsr;
...
#endif
int acp3400_rio_apio_enable(struct rio_mport *mport, u32 mask, u32 bits)
{
	struct rio_priv *priv = mport->priv;
	int rc;
...
EXPORT_SYMBOL_GPL(acp3400_rio_apio_enable);
int acp3400_rio_apio_disable(struct rio_mport *mport)
{
	struct rio_priv *priv = mport->priv;
...
EXPORT_SYMBOL_GPL(acp3400_rio_apio_disable);
int acp3400_rio_rpio_enable(struct rio_mport *mport, u32 mask, u32 bits)
{
	struct rio_priv *priv = mport->priv;
	int rc = 0;
...
EXPORT_SYMBOL_GPL(acp3400_rio_rpio_enable);
int acp3400_rio_rpio_disable(struct rio_mport *mport)
{
	struct rio_priv *priv = mport->priv;
...
EXPORT_SYMBOL_GPL(acp3400_rio_rpio_disable);
