...
/**
 * acp_rio_tx_db_int_handler - AXXIA inbound doorbell interrupt handler
...
static void ob_dme_irq_handler(struct rio_irq_handler *h, u32 state)
{
	...
	do {
		...
		for (i = 0; i < mbox->entries; i++) {
			...
			if (priv->descriptors) {
				dw0 = *((u32 *)DESC_TABLE_W0_MEM(priv,
								desc->desc_no));
			} else {
	...
	for (i = 0; i < mbox->entries; i++) {
		...
		if (priv->descriptors) {
			dw0 = *((u32 *)DESC_TABLE_W0_MEM(priv, desc->desc_no));
		} else {
			__rio_local_read_config_32(mport,
					DESC_TABLE_W0(desc->desc_no), &dw0);
		}
		...
		if ((dw0 & DME_DESC_DW0_VALID) &&
			...
			if (priv->descriptors) {
				*((u32 *)DESC_TABLE_W0_MEM(priv, desc->desc_no))
			  ...
			  int entries, int prio)
{
	...
	for (i = 0, desc = me->desc; i < entries; i++, desc++) {
		if (priv->descriptors) {
			*((u32 *)DESC_TABLE_W0_MEM(priv, desc->desc_no)) = 0;
			*((u32 *)DESC_TABLE_W1_MEM(priv, desc->desc_no)) = 0;
			*((u32 *)DESC_TABLE_W2_MEM(priv, desc->desc_no)) =
				((u32)(desc->msg_phys >> 8)) & 0x3fffffff;
			*((u32 *)DESC_TABLE_W3_MEM(priv, desc->desc_no)) = 0;
		} else {
			__rio_local_write_config_32(mport,
	...
	if (priv->descriptors) {
		...
		*((u32 *)DESC_TABLE_W3_MEM(priv, desc->desc_no)) = dw3;
		...
		__rio_local_read_config_32(mport,
				    DESC_TABLE_W2(desc->desc_no),
				    &dw2);
		dw2 |= ((DESC_TABLE_W0(me->dres.start) >> 6) & 0xc0000000);
			 ...
			 int mbox, int entries)
{
	...
	if (!mb)
		return -ENOMEM;
	mb->mbox_no = mbox;
	mb->virt_buffer = kzalloc(priv->desc_max_entries * sizeof(void *),
				  GFP_KERNEL);
	if (!mb->virt_buffer) {
		kfree(mb);
		return -ENOMEM;
	...
	for (letter = 0; letter < RIO_MSG_MAX_LETTER; ++letter) {
		...
		for (i = 0, desc = me->desc; i < entries; i++, desc++) {
			...
			if (priv->descriptors) {
				*((u32 *)DESC_TABLE_W0_MEM(priv,
						 desc->desc_no)) = dw0;
				*((u32 *)DESC_TABLE_W1_MEM(priv,
						 desc->desc_no)) = dw1;
				*((u32 *)DESC_TABLE_W2_MEM(priv,
						 desc->desc_no)) = dw2;
				*((u32 *)DESC_TABLE_W3_MEM(priv,
						 desc->desc_no)) = dw3;
			} else {
				__rio_local_write_config_32(mport,
					DESC_TABLE_W0(desc->desc_no), dw0);
			     ...
			     void *buffer, size_t len, void *cookie)
{
	...
	if (priv->descriptors) {
		*((u32 *)DESC_TABLE_W1_MEM(priv, desc->desc_no)) = dw1;
		*((u32 *)DESC_TABLE_W0_MEM(priv, desc->desc_no)) = dw0;
	} else {
		__rio_local_write_config_32(mport,
					 DESC_TABLE_W1(desc->desc_no), dw1);
			    ...
			    int *sz, int *slot, u16 *destid)
{
	...
	if (!in_interrupt() &&
		...
		__rio_local_write_config_32(mport, RAB_INTR_ENAB_IDME,
					    intr & ~(1 << mbox));
		ib_dme_irq_handler(&priv->ib_dme_irq[mbox], 1 << mbox);
