...
//STATIC int ubootenv_update( void );
STATIC int ubootenv_read( struct mtd_info *, size_t, void * );
//STATIC int ubootenv_write( struct mtd_info *, size_t, void * );
...
/*
STATIC int
ubootenv_write( struct mtd_info * mtd, size_t size, void * buffer )
{
	DEBUG_PRINT( "Erasing %s\n", mtd->name );
	return 0;
}
*/
#ifdef DEBUG_EXPORT_SYMBOLS
 //EXPORT_SYMBOL( ubootenv_write );
#endif /* DEBUG_EXPORT_SYMBOLS */
...
ubootenv_initialize( void )
{
	...
	*/
	...
	if( ( environment_t * ) 0 ==
	    ( env0 = ( environment_t * ) vmalloc( uboot_env_size ) ) ) {
		ERROR_PRINT( "Unable to allocate %lu bytes\n", uboot_env_size );
		return -1;
	}

	if( ( environment_t * ) 0 ==
	    ( env1 = ( environment_t * ) vmalloc( uboot_env_size ) ) ) {
		ERROR_PRINT( "Unable to allocate %lu bytes\n", uboot_env_size );
		vfree( ( void * ) env0 );
		return -1;
	}
...
#ifdef DEBUG_EXPORT_SYMBOLS
EXPORT_SYMBOL( ubootenv_initialize );
#endif /* DEBUG_EXPORT_SYMBOLS */
...
/*
STATIC int
ubootenv_update( void )
{
	if( NULL == environment ) {
		ERROR_PRINT( "No Environment Available to Write.\n" );
		return -1;
	}

	if( -1 == uboot_env_current ) {
		ERROR_PRINT( "Environment Copy Selection is Invalid.\n" );
		return -1;
	}

	if( 0 == uboot_env_current ) {
		DEBUG_PRINT( "Saving Environment in Copy 1.\n" );
	} else {
		DEBUG_PRINT( "Saving Environment in Copy 0.\n" );
	}
...
#ifdef DEBUG_EXPORT_SYMBOLS
EXPORT_SYMBOL( ubootenv_finalize );
#endif /* DEBUG_EXPORT_SYMBOLS */
			...
			char *buf, loff_t off, size_t size)
{
	...
	while( 0x00 != string [ 0 ] ) {
		...
		if( 0 == strncmp( bin_attr->attr.name, string, strlen( bin_attr->attr.name ) ) ) {
			char * value_ = strchr( string, '=' );
			++ value_;
...
STATIC void create_env_sysfs(void)
{
	...
	while( 0x00 != string [ 0 ]) {
		...
		if (sysfs_create_bin_file(ubootenv_kobj, &uboot_env[i])) {
			ERROR_PRINT("unable to add uboot-env "
				    "sysfs file\n");
		}
